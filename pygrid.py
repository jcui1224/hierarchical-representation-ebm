...
def run_job(logger, opt, output_dir, train):
    ...
    try:
        ...
        try:
            ...
            p = multiprocessing.Process(target=train, args=(opt, output_dir_thread, return_dict))
            p.start()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                logger.info('assertion failed in run_jobs()')
                update_job_status(opt['job_id'], 'fail', read_opts, write_opts)
            except Exception:
                logger.info('exception in run_jobs()')
                update_job_status(opt['job_id'], 'fail', read_opts, write_opts)
